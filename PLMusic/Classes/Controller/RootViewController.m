...
-(void) playSongSetting {
    ...
    __weak typeof(MusicPlayerManager) *weakmp = musicPlayer;
    __weak typeof(self) weakSelf = self;
    
    __weak typeof(OMSongInfo) *weakoi = songInfo;
    // 设置Observer更新播放进度
    _playerTimeObserver = [musicPlayer.play addPeriodicTimeObserverForInterval:CMTimeMake(1.0, 1.0) queue:dispatch_get_main_queue() usingBlock:^(CMTime time) {
        ...
        CMTime total = weakmp.play.currentItem.duration;
        CGFloat totalTime = CMTimeGetSeconds(total);
        
        // 当前播放时间
        weakSelf.detailController.bottomView.currentTimeLabel.text = [weakoi intToString:(int)currentTime];
        // 总时间
        weakSelf.detailController.bottomView.durationTimeLabel.text =[weakoi intToString:(int)totalTime];
